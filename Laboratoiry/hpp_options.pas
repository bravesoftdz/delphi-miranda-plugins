...
begin
  ...
  fid.deffontsettings.colour := Color2RGB(hppFontItems[Order].color);
  FontRegisterW(@fid);
end;
...
begin
  ...
  cid.defcolour := Color2RGB(hppFontItems[Order].back);
  ColourRegisterW(@cid);
end;
...
var
  ...
  Len: Integer;
begin
  Len := GetFullPathNameW(PWideChar(FileName), Length(Buffer), @Buffer, FName);
  if Len <= Length(Buffer) then
    SetString(Result, Buffer, Len)
  else if Len > 0 then
  begin
    SetLength(Result, Len);
    Len := GetFullPathNameW(PWideChar(FileName), Len, PWideChar(Result), FName);
    if Len < Length(Result) then
      SetLength(Result, Len);
  end;
end;

function FindIconsDll(ForceCheck: boolean): WideString;
var
  hppIconsDir: WideString;
  hppMessage: WideString;
  CountIconsDll: Integer;
  DoCheck: boolean;
  hppDllName,hppPluginsDir:WideString;
begin
  // Get plugins dir
  ...
  begin
    ...
    begin
      ...
      HppMessageBox(0{hppMainWindow}, hppMessage, hppName + ' Error', MB_ICONERROR or MB_OK);
    end;
  end;
  if DoCheck then
  begin
    ...
    begin
      hppMessage :=
        ...
        [Result]);
      HppMessageBox(0{hppMainWindow}, hppMessage, hppName + ' Warning', MB_ICONWARNING or MB_OK);
    end;
  end;
end;
...
begin
  ...
  sid.szDefaultFile.w := PWideChar(hppIconPack);
  ...
  begin
    ...
    if hppIconsDefs[i].group = nil then
      ...
      sid.szSection.w := PWideChar(WideString(hppName + '/') + hppIconsDefs[i].group);
  ...
  begin
    ...
    begin
      ...
      sid.szDescription.w := PWideChar(EventRecords[mt].name);
      sid.szSection.w := PWideChar(WideString(hppName + '/' +'Events'));
