...
type
  ...
  tagMACHINE_ID = packed record
    szName: packed array[0..15] of AnsiChar;
  end;
...
begin
  ...
  begin
    ...
    begin
      _SHCreateItemFromIDList := GetProcAddress(Shell32Lib, PAnsiChar('SHCreateItemFromIDList')); // Do not localize
      if Assigned(_SHCreateItemFromIDList) then
        Result := _SHCreateItemFromIDList(pidl, riid, ppv);
    end;
  end;
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHCreateItemFromParsingName := GetProcAddress(Shell32Lib, PAnsiChar('SHCreateItemFromParsingName')); // Do not localize
      if Assigned(_SHCreateItemFromParsingName) then
        Result := _SHCreateItemFromParsingName(pszPath, pbc, riid, ppv);
    end;
  end;
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHCreateItemWithParent := GetProcAddress(Shell32Lib, PAnsiChar('SHCreateItemWithParent')); // Do not localize
      if Assigned(_SHCreateItemWithParent) then
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHCreateItemFromRelativeName := GetProcAddress(Shell32Lib, PAnsiChar('SHCreateItemFromRelativeName')); // Do not localize
      if Assigned(_SHCreateItemFromRelativeName) then
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHCreateItemInKnownFolder := GetProcAddress(Shell32Lib, PAnsiChar('SHCreateItemInKnownFolder')); // Do not localize
      if Assigned(_SHCreateItemInKnownFolder) then
        Result := _SHCreateItemInKnownFolder(kfid, dwKFFlags, pszItem, riid, ppv);
    end;
  end;
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHGetIDListFromObject := GetProcAddress(Shell32Lib, PAnsiChar('SHGetIDListFromObject')); // Do not localize
      if Assigned(_SHGetIDListFromObject) then
        Result := _SHGetIDListFromObject(punk, ppidl);
    end;
  end;
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHGetPropertyStoreFromIDList := GetProcAddress(Shell32Lib, PAnsiChar('SHGetPropertyStoreFromIDList')); // Do not localize
      if Assigned(_SHGetPropertyStoreFromIDList) then
        Result := _SHGetPropertyStoreFromIDList(pidl, flags, riid, ppv);
    end;
  end;
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHGetPropertyStoreFromParsingName := GetProcAddress(Shell32Lib, PAnsiChar('SHGetPropertyStoreFromParsingName')); // Do not localize
      if Assigned(_SHGetPropertyStoreFromParsingName) then
...
begin
  ...
  begin
    ...
    if Shell32Lib > 0 then
    begin
      _SHGetNameFromIDList := GetProcAddress(Shell32Lib, PAnsiChar('SHGetNameFromIDList')); // Do not localize
      if Assigned(_SHGetNameFromIDList) then
        Result := _SHGetNameFromIDList(pidl, sigdnName, ppszName);
    end;
  end;
